...
Schema 29 adds `reference_postings_navigation_names_idx`, a partial covering
index on `(normalized_name, raw_name, reference_kind)` for `target_type = 0`.
The navigation reference-name reader is answered entirely from it: no table
lookup, no `pages` join, and no temp B-tree for either the `DISTINCT` or the
`ORDER BY`. That reader also now yields one row per distinct spelling rather
than one per (page, spelling), which is a breaking Rust API change but no
change to stored bytes beyond the index itself. Normal and terminal
construction produce the same index. The Direct query metadata marker advances
to 29 too: an older Direct reader validates the `direct_source_revisions` DDL
text but tolerates additional indexes, so without that move it would accept a
schema-29 cache instead of rebuilding it. Existing projections are refused on
the exact `user_version` and rebuilt from Markdown/Org through the ordinary app
lifecycle; this change adds no migration or compatibility reader.
